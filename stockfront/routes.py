...
from stockfront.models import User, Upvote, WatchlistStock
...
#Globals
serverSite = '<stockdatadatabase>'
serverSiteUser = '<usersdatabase>'
iexKey = "<apikey>"
  ...
  if  form1.validate_on_submit():
      ...
      if ticker in tickers:
        ...
        return render_template("stock.html",  stock = stock, dates = json.dumps(list(latest_data.date)), close = list(latest_data.close), form1 = form1, newsdata = newsdata,
                                float = format(stockdata.float,',d'), pe = stockdata.peRatio, marketcap = format(stockdata.marketcap,',d'), beta = round(stockdata.beta,2),
